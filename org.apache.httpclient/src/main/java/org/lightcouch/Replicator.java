...
public class Replicator {
    ...
    public void removeReplicatorCouchDB(CouchDbClient target)
    {
        dbc.replicator().source(dbc.getBaseUri()+"/"+dbc.getDbName()).target(target.getBaseUri()+"/"+target.getDbName()).remove();
    }

    public void addReplicatorCouchDB(CouchDbClient target,boolean continuous)
    {
        if(continuous)
        {
            Replicator re = dbc.replicator().source(dbc.getBaseUri()+"/"+dbc.getDbName()).target(target.getBaseUri()+"/"+target.getDbName()).continuous(true);
                            re.save();
        }  else {
            dbc.replicator().source(dbc.getBaseUri()+"/"+dbc.getDbName()).target(target.getBaseUri()+"/"+target.getDbName()).save();
        }
    }

    //curl -H 'Content-Type: application/json' -X POST -d '{"source":"http://192.168.1.113:8888/jed","target":"jed", "continuous":true}' http://192.168.1.121:8888/_replicate
    public HttpResponse addreplicatorTouchDB(CouchDbClient target,boolean  continuous)
    {
        ...
        try
        {
            String json="";
            if(continuous){
                json = "{\"source\":\""+target.getBaseUri()+dbc.getDbName()+"\",\"target\":\""+target.getDbName()+"\", \"continuous\":true}'";
            }else {
                json = "{\"source\":\""+target.getBaseUri()+dbc.getDbName()+"\",\"target\":\""+target.getDbName()+"\"}'";
            }
            URI uri = builder(dbc.getBaseUri()).path("_replicate").build();
            response = dbc.post(uri,json);
        }catch (Exception e)
        {
            return null;
        } finally {
